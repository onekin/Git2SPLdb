...
public class MineProductPortfolios implements CommitVisitor {
	...
	public void process(SCMRepository repo, Commit commit, PersistenceMechanism writer) {
		...
		//	if (commit.getBranches().contains(Customs.coreAssetsBranchPatternName)){ //this commit is the origin of the product
				...
				while(it.hasNext()){
				  ...
				  {
					  p = new Product(customDiff.utils.Utils.getNewProductId(), commit, brName, pp);
	...
	public void computeAllReleasesForProduct(Product p, SCMRepository repo) {
		  try{
			  ...
			  Iterable<RevCommit> revCommits = git.log().add(repository.resolve(p.getName())).call();
			  
			  System.out.println("Commits for branch: "+p.getName());
	...
	private void mineProductAssetsForProductRelease(ProductRelease pr1, SCMRepository repo) {
		try{
			...
			for(RepositoryFile file : files) {//Mining Files for a product release
				...
				PAFile= new ProductAssetFileAnnotated(customDiff.utils.Utils.getNewProductAssetId(), file.getFile().getName(),  file.getFile().getPath(), file.getSourceCode(),
				...
				HashMap<Integer, ArrayList <String>> map = customDiff.utils.FeatureAnalysisUtils.extractFeatureMapFromFile(PAFile, pr1.getFromProduct().getInPortfolio().getDerivedFrom()); //line-feature map
				ArrayList<VariationPoint> vps = customDiff.utils.FeatureAnalysisUtils.extractVPsFromFile(PAFile, pr1.getFromProduct().getInPortfolio().getDerivedFrom());
