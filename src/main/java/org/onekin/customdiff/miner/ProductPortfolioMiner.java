...
import org.onekin.customdiff.spldomain.ProductAssetFileAnnotated;
	        ...
	        while (treeWalk.next()) {
	            ...
	            if(path.contains(inPath)){
	              ...
	              file= new ProductAssetFileAnnotated(Utils.getNewAssetId(), treeWalk.getNameString(), treeWalk.getPathString(), fileContent,
		    				  fileContent.split("\n").length, pr, 
		    				  CustomDiff.pathToWhereCustomizationsAreComputed.concat(treeWalk.getPathString().split(CustomDiff.pathToWhereCustomizationsAreComputed)[1]),
