...
import org.onekin.customdiff.spldomain.Product;
...
import org.onekin.customdiff.spldomain.ProductPortfolio;
...
public class ExportToMySQLDatabase implements ExportTarget {
	...
	//	utils.FileUtils.writeToFile(this.pathToDataFile+"0insertsSPL.sql",inserts0);
		...
		FileUtils.writeToFile(this.pathToDataFile+"data-inserts.sql",allInserts);
	...
	private String generateInsertsFor_FeatureInVariationPoint_table() {
		...
		List<SourceCodeFile> assets ;
		List<Product> products;
		List<ProductPortfolio> portfolios = CustomDiff.spl.getProductPortfolioList();
	...
	private String generateInsertsFor_VariationPoint_table() {
		...
		List<VariationPoint> vps;
		VariationPoint vp;
		List<CoreAssetBaseline> baselines = CustomDiff.spl.getCoreAssetBaselines();
		List<SourceCodeFile> assets = null;
	...
	private String generateInsertsFor_ProductPortfolio_Table() {
		...
		ArrayList<ProductPortfolio> list = CustomDiff.spl.getProductPortfolios();
		
		for (int i=0; i < list.size(); i++){
			insert=insert.concat(header).concat("('"+list.get(i).getPortfolioID()+"','"+ CustomDiff.spl.getId()+"','"+list.get(i).getDerivedFrom().getId()+"');\n");
	...
	private String generateInsertsFor_Product_Table() {
		...
		ArrayList<ProductPortfolio> portfolios = CustomDiff.spl.getProductPortfolioList();
	...
	private String generateInsertsFor_CoreAsset_has_Feature_Table() {
		...
		List<CoreAssetBaseline> baselines = CustomDiff.spl.getCoreAssetBaselines();
		List<SourceCodeFile> caList = null;
		
		for(int i=0; i< baselines.size();i++){
			caList=baselines.get(i).getCoreAssetFiles();
			for(int j=0; j< caList.size();j++){
				List<Feature> features = caList.get(j).getFeatureList();
	...
	private String  generateInsertsFor_CoreAsset_Baseline_has_Feature() {
		...
		List<CoreAssetBaseline> baselines = CustomDiff.spl.getCoreAssetBaselines();
		List<Feature> featuresInBaseline;
	...
	private String generateInsertsFor_Feature_Table() {
		...
		ArrayList<Feature> list = CustomDiff.features;
		if (list==null) return "";
	...
	private String generateInsertsFor_CoreAssetBaseline_Table() {
		...
		ArrayList<CoreAssetBaseline> list = CustomDiff.spl.getCoreAssetBaselines();
		if (list==null) return "";
		...
		while (it.hasNext()){
			b = it.next();
			insert=	insert.concat(header).concat("(" +"'"+b.getId()+"'"+","+"'"+convertDateToMysqlForm (b.getReleaseDate())+"'"+","+"'"+ CustomDiff.spl.getId() +"'"  +");\n");
		}
		return insert;
	}
	...
	private String generateInsertsFor_SPL_Table() {
		...
		insert=insert.concat ("('"+ CustomDiff.spl.getId()+ "')");
