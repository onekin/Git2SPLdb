...
CREATE TABLE IF NOT EXISTS `olapCustoms`.`product_release` (
  ...
  INDEX `fk_ProductRelease_Product1_idx` (`idproduct` ASC),
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `olapCustoms`.`churn_productportfolio_features` (`'id'` INT, `'id_feature'` INT, `'featuremodified'` INT, `'id_product'` INT, `'id_pr'` INT, `'pr_name'` INT, `'added'` INT, `'deleted'` INT, `'churn'` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `olapCustoms`.`coreassets_and_features` (`'id'` INT, `'idcoreasset'` INT, `'caname'` INT, `'capath'` INT, `'idfeature'` INT, `'size'` INT);

-- -----------------------------------------------------
-- Placeholder table for view `olapCustoms`.`customs_by_feature_and_coreasset`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `olapCustoms`.`customs_by_feature_and_coreasset` (`'id'` INT, `'idcustomization'` INT, `'idfeature'` INT, `'idcoreasset'` INT, `'caname'` INT, `'capath'` INT, `'idproductrelease'` INT, `'prname'` INT, `'added'` INT, `'deleted'` INT, `'custom_diff'` INT, `'messages'` INT, `'commits'` INT, `'maindiff'` INT);
...
select UUID() as 'id',f.idfeature as 'id_feature' ,f.name as 'featuremodified', p.idproduct as 'id_product', pr.idproductrelease as 'id_pr', pr.name as 'pr_name',
...
inner join feature f on f.idfeature = fb.id_feature
inner join product_release pr on pr.idproductrelease = c.idproductrelease
inner join product p on p.idproduct = pr.idproduct
group by f.idfeature, pr.idproductrelease, p.idproduct;
...
inner join product_release pr on pr.idproductrelease = c.idproductrelease
inner join product p on p.idproduct = pr.idproduct
group by f.idfeature, pr.idproductrelease, p.idproduct, ca.idcoreasset;
...
select UUID() as 'id', ca.idcoreasset as 'idcoreasset', ca.name as 'caname', ca.path as 'capath',
...
select UUID() as 'id', c.idcustomization as 'idcustomization', f.idfeature as 'idfeature', ca.idcoreasset as 'idcoreasset', 
